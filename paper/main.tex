...
\begin{enumerate}
\item A leakage-audited protocol for interval evaluation on C-MAPSS (Section~\ref{sec:protocol}), with a controlled $2\times2$ retraining experiment, the fitting engines held fixed, separating checkpoint selection from normaliser scope.
...
\begin{table*}[!t]
\caption{Controlled $2\times2$ protocol experiment, heteroscedastic LSTM, terminal unit, mean of five seeds. All four cells are trained on the same fitting engines (the seed's 60\% fitting set); only the checkpoint criterion and the normaliser scope differ. Sel.: checkpoint selected on test set (T) or validation engines (V). Norm.: normaliser fitted on whole training file (W) or fitting engines only (F). Sel.\ is the mean over normaliser levels of (T$-$V), Norm.\ the mean over selection levels of (W$-$F), and Int.\ the difference of differences (T/W$-$T/F)$-$(V/W$-$V/F), all on the RMSE/score scale; negative means the choice makes the result look better.}
\label{tab:leak}
\centering\small
...
Holding the fitting engines fixed separates the two choices, and they are not of comparable size. Selecting the checkpoint on the test set improves RMSE by \NSelEffRMSE{} cycles and the NASA score by \NSelEffScore{} units; restricting the normaliser to the fitting engines changes RMSE by \NNormEffRMSE{} cycles, at most \NNormAbsMaxRMSE{} of a cycle, and the score by \NNormEffScore. The interaction, \NIntEffRMSE{} cycles and \NIntEffScore{} score units, is of the same order as the normaliser effect and larger than it on FD004. The only cell in which a leaked choice does not flatter the result is the \NLeakExceptions. From the fully leaked to the fully clean configuration the combined effect reaches \NLeakRMSEmax\% in RMSE and \NLeakScoremax\% in the NASA score on FD001, larger than the margins by which recent methods are separated; on the six-condition sub-datasets it is a few percent.

An earlier, uncontrolled version of this experiment left the fitting set free, so that the whole-file cells were also fitted on more engines; the normaliser then appeared to cost as much as the selection choice. It does not. That apparent effect was the training-set size, which is why the controlled design is the one reported here, and why a reader comparing protocols across papers should check what else changes with them.

The interval metrics move differently. Test-set selection changes $\PICP$ by \NSelPICP on the three sub-datasets and raises $\ECE$ on all three, by 0.003 to 0.021. The selection criterion is RMSE, a statistic of the mean alone; the variance head is a bystander to the search, and whatever the search does to calibration is a side effect. A leaked point-accuracy figure is optimistic; a leaked interval figure is merely unreliable.
...
The code and results are released at \url{https://github.com/jeff-1980/rul-interval-reliability}, tag \texttt{v1.8-submission}; C-MAPSS is available from the NASA Prognostics Center of Excellence. Reproduction is supported at two levels. (i) From the released result files, a single script (\texttt{paper/make\_tables.py}) regenerates every table and every number quoted in this paper and its supplement; this is the level we claim is reproducible by a reader. (ii) From the released checkpoints, the inference scripts regenerate those result files. Two independent runs of this pipeline on the machine described (PyTorch 2.11, CUDA 13, NVIDIA RTX A5000 Laptop GPU) reproduced \NReproBit{} result files bit-for-bit and the remainder in every field except a wall-clock timing field, with hashes listed in \texttt{REPRODUCE.md}. The pipeline also reads a small number of intermediate files from earlier development runs that are not part of the release, and we do not claim that it runs unchanged on another machine. Model training, latency measurement and cross-platform reproduction are outside both levels. Development records, including the superseded pre-audit runs described in Appendix~\ref{app:audit}, are retained by the author and are available on reasonable request.
