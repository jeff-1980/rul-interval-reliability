...
if __name__ == '__main__':
    ...
    for ds in C.DATASETS:
        ...
        picp_nll_per_seed, picp_mc_per_seed, picp_cp_per_seed = [], [], []
        ...
        for si, seed in enumerate(C.SEEDS):
            ...
            ens_mu_members.append(mu); ens_sigma_members.append(sigma)  # reuse NLL fwd pass for Ensemble member
            del model

            # MC-Dropout
            ckpt = torch.load(os.path.join(CKPT_DIR, f"{ds}_MCDropoutMSE_seed{seed}.pt"),
            ...
            q_norm = cp_json[ds][si]['cp_norm']['q']
        ...
        results[ds] = method_results

    out_path = os.path.join(RESULTS_DIR, 'per_engine_coverage_leakfree.json')
