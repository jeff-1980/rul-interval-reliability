...
if __name__ == '__main__':
    ...
    print(f"Device: {device}  DS={DS}  M_LEVELS={M_LEVELS}  total seeds={len(ALL_15_SEEDS)}  scheme=global")
    ...
    for M in M_LEVELS:
        ...
        for gi, group in enumerate(groups):
            ...
            for snr in SNR_LEVELS:
                ...
                raw_noisy = V4.inject_noise_raw(test_df_raw, feat_cols, snr, noise_rng, 'global',
    ...
    out_path = os.path.join(LEAKFREE_DIR, 'FD003_ensemble_scale_sweep_leakfree.json')
    with open(out_path, 'w') as fp:
        json.dump({DS: ds_out}, fp, indent=2, default=float)
    print(f"\nSaved -> {out_path}")
    print("STEP FD003-2d complete.")
